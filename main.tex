...
The out-of-sample performances of the models can be found in Figure \ref{analysis1}A,B. Using mixed training data (i.e., acute and chronic subjects) improved predictions for higher-dimensional ChaCo models
...
We saw that simple linear regression models based on the extent of lesion damage to lesion-behavior maps outperformed all theory-based biomarkers representing damage to tracts in the sensorimotor system. Many voxels, particularly in the right hemisphere, that lie outside of these sensorimotor tracts are present in the lesion-behavior map (Figure \ref{lbm_vs_smatt}). Shifts in lesion-behavior association maps towards vascular territories have been well-documented \cite{Mah2014-cb, Sperber2020-kp}) and have been considered artefactual displacements that emerge due to the non-uniform distribution of lesions. This study suggests that, regardless of whether these lesion-behavior maps reflect areas that are causally related to a deficit, the extent of lesion damage to these maps is a more useful biomarker for the prediction of chronic motor deficits than the extent of damage to elements of the motor system as we currently understand it. 
