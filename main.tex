...
\cite{Bowren2022-rs} discover such associations using lesion-behavior mapping, a technique to anatomically localize functions in the brain using lesion data. They seed structural connectivity from regions of peak association and use damage to those structural connectivity maps as input to predictive models, in a new cohort. Similarly, \cite{Sperber2021-lw} and \cite{Rondina2016-ds} find that damage to regions identified from lesion-behavior mapping can be used to predict motor deficits. This prior work suggests that data-driven feature selection enhances predictive performance. However, using inferential statistics to identify features (i.e. from associative lesion-behavior mapping methods) may fail to identify features that are predictive, i.e., are useful for estimating motor deficits in new subjects (\cite{Bzdok2020-py}). For instance, combined information from several variables that are not significantly associated with an outcome may be required to achieve the higher prediction performances, or silencing of a nuisance variable may be required for optimal predictions.
...
Sensorimotor tract segmentations were obtained from the sensorimotor area tract template (SMATT) (\cite{Archer2018-ti}), a set of 12 tracts derived from probabilistic tractography seeded in the left and right primary motor cortex (M1), dorsal and ventral premotor cortex (PMd and PMv, respectively), supplementary motor area (SMA), pre-supplementary motor area (pre-SMA), and primary somatosensory cortex (S1) of healthy controls (Supplementary Figure \ref{smatt}). Lesion load was calculated as above, for all 12 tracts (i.e. separately for left and right hemispheres) and for 6 ipsilesional tracts. Left and right SMATT lesion load was calculated in order to assess whether preserving hemispheric information improved predictions. For subjects with brainstem, cerebellar, and/or bilateral cerebral strokes (N=30 chronic, N=43 acute), ipsilesional lesion load was calculated as the average lesion load of the left and right hemisphere tracts. Lesion loads also had a heavy-tailed distribution (Supplementary Figure \ref{lesion_load_dist}B,C). 
...
Beta coefficients for ChaCo models are shown in (Figure \ref{analysis1}C, D) (reflecting features selected in least 250/500 outer folds of the shen268 models). There were several similarities in the pattern of feature weights in the fs86 ChaCo model and shen268 ChaCo model with feature selection. For both atlases, negative model weights are assigned to superior portions of left and right motor areas, as well as in subcortical structures like the putamen, pallidum, and thalamus, whereas positive weights are assigned to frontal and parietal areas.

In the shen268 ChaCo models with feature selection, on avearge 110 features were selected (Supplementary Figure \ref{lambda_kappa}). A hierarchically-clustered heatmap of the correlation matrix of the selected features (Supplementary Figure \ref{heatmap_shen}) shows that many of the selected features are clustered (i.e., high inter-feature correlation across subjects), reflecting some redundancy in the variables selected. 
...
\section{Discussion}
Confirms previous studies that M1-LL does not capture enough variance in wide range of lesion topographies. Although damage to non-primary motor cortex CST improves predictions, it is still lower than what can be achieved with data-driven feature selection.

Why did ChaCo models perform better than lesion load models? ChaCo scores are able to capture white matter and grey matter damage in one measurement, which may enable it to capture associations between grey matter damage and motor disruption that are not captured with lesion load metrics that primarily reflect white matter damage (\cite{Rondina2016-ds} find similar importance of cortical GM regions).

Data-driven feature selection for prediction diverges from measures of association (\cite{Sperber2021-lw}). In particular, from the mass-univariate correlation between ChaCo scores and normed motor scores one may conclude that damage across the cortex and in subcortical structures is associated with worse motor deficits. However, models that maximize predictive accuracy of deficits in new subjects must take into account the effect of damage to multiple regions and how damage to one area is related to damage at every other area. The feature weights in Figure \ref{analysis1}C, D are not meant to be maps of neural correlates of hemiparesis; rather, they are a pattern of weights that, when linearly multiplied with regional ChaCo scores, explain the most variance in chronic motor scores.

Still thinking about why sLNM failed (\cite{Bowren2022-rs}). sLNM-LL models performed worse than SMATT-LL models. Although the Left/right hemisphere SMATT-LL models had more features (12) than sLNM-LL models (5), ipsilesional SMATT-LL models were comparable, containing 6 features. One additional feature is unlikely to confer, in itself, the discrepancy in model performance; differences in the information captured by the features likely contribute to model accuracy. Ipsilesional SMATT-LL models capture variance in lesion damage across separate descending branches of the corticospinal tract, while sLNM-LL models reflect a coarse measure of damage to white matter around and including the CST. Preserving variance of damage around the descending white matter tracts may be important for predictions of chronic motor deficits.

The feature selection strategy used by \cite{Bowren2022-rs} is lesion-deficit mapping using sparse canonical correlations analysis, an approach that finds linear combinations of lesion data and deficits that have maximum correlation with each other (\cite{Pustina2018-xv}). Although multivariate, this is still an inferential modelling approach to understand causal relationships between lesion damage and behavior, albeit in low-dimensional space. The relatively poor performance of sLNM compared to data-driven feature selection suggests that imaging features derived from an inferential framework, though critical for localization and understanding post-stroke deficits, may not generalize to new subjects as well as features selected specifically for that purpose.

Feature selection only improved model performance for ChaCo scores of then Shen atlas, reflecting structural disconnection of 268 finely-parcellated gray matter regions. Two aspects of this model likely contributed to its success: first, correlation-based feature selection identified patches of correlated damage (hence the observed clustering in the correlation matrix of selected predictors) (discussino on patch size: \cite{Pustina2018-xv}). Second, the small size of parcels in the shen268 atlas allowed for preservation of signal in relevant individual gray matter regions. Because the shen268 atlas is derived from functional atlas, these parcels are likely close in shape to real functional motor areas. This preserved, unique signal in individual regions can also be inferred from the correlation matrix of selected predictors, where several regions are consistently incorporated into the model but have low correlation with other selected regions. The optimal number of deficit-predictive parcels is left to future work.  On the other hand, the 86-region FreeSurfer parcellation may have been too coarse, and relevant signal may have been diluted across large gray matter regions, or more components were needed to adequately model the relationship between lesion damage and deficits.

Can lesion-behavior mapping and post-stroke outcome prediction converge? Curiously, Mah et al. and Sperber et al. 2020 Cortex use SVM for lesion-behavior mapping, inferring causal maps of hemiparesis from a predictive framework, which identified features outside of critical areas of interest (Sperber). To what extent are predictive, non-causal associations between lesions and impairment generalizable? Here, we show that they are at least somewhat generalizable.




Would a perfectly unbiased map of voxels that, when damaged, cause hemiparesis, be the most ideal? to what extent can useful/predictive lesion-anatomical associations apply with covariate shift? because this dataset contains many subjects from multiple imaging sites, perhaps these features have already been found.


Are features that are predictive of motor deficits causally related to hemiparesis?

This result paves the way for future work to model lesion-deficit associations hierarchically.

In summary, not only do we need to consider the contribution of non-motor regions to the prediction of motor scores because of the topographical irregularities of lesion data, but that inferential methods may be merely characterizing the mislocalization effect. There is yet another level of association that we must consider, which is the subset of predictors that are critical for predictions. These may be (but are not necessarily) the regions where a lesion-deficit relationship exists. 

Inclusion of the acute data improved predictions. Although in this paper we did not attempt to disentangle the effect of greater training size from relevance of the signal in the acute data, we can say that the acute data was at least not harmful. In particular, feature selection also improved performance using the acute data. If the statistical relationships between lesion and deficits changed over the course of the stroke then this information may not be useful. Therefore there is some helpful signal in this population. Likely, unmeasured factors like therapy, comorbidities, and lifestyle weaken the underlying relationship between lesion location and motor deficit as time between the stroke and the assessment increases. We may also conclude that because of this shared variance, predictive performance in acute subjects can be thought of as an upper limit of performance achieveable on chronic data.


Ensemble results: averaging predictions from multiple models improves perofrmance above either model alone. This suggests that the information within each is not redundant. 
...
\caption{Sum of regional change in connectivity (ChaCo) scores across chronci subjects. Sum of ChaCo scores parcellated with the FreeSurfer 86-region atlas (\textbf{A}) and with the Shen 268-region atlas (\textbf{B}, normalized to the maximum value across regions (red = higher sum of ChaCo score across subjects)}
...
\caption{Hierarchically-clustered heatmap of the correlation matrix of all selected features (ChaCo scores) chosen in at least 99 percent of outer folds by correlation-based feature selection in ridge regression using the shen 268-region atlas. Heatmap (top left) displays values corresponding to correlation between features across subjects. Regions are labeled in the following format: "Hemisphere, lobe: Yeo Network".}
\label{heatmap_shen}
\end{figure}
