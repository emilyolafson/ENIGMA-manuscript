...
In this paper, we robustly compare the predictive accuracy of several imaging biomarkers of post-stroke motor impairment by assessing their out-of-sample performance using the ENIGMA dataset, a multi-site stroke lesion database (\cite{Liew2020-ps}). We compare theory-driven features (i.e. motor-related CST lesion load) with data-driven imaging features. As the time between the stroke and the motor assessment increases, external factors may influence the relationship between lesion location and motor outcome. Models trained on chronic stroke subjects may therefore struggle to identify signal related to motor outcomes. We assessed whether models trained on data from acute subjects predict motor outcomes of chronic subjects better than models trained on chronic stroke data.
...
Corticospinal tract segmentations were obtained from the sensorimotor area tract template (SMATT), a set of 6 tracts derived from probabilistic tractography seeded in primary motor cortex (M1), dorsal and ventral premotor cortex (PMd and PMv, respectively), supplementary motor area (SMA), pre-supplementary motor area (pre-SMA), and primary somatosensory cortex (S1) (Figure \ref{smatt_template}) (\cite{Archer2018-ti}). Lesion load for each tract was calculated as:
\begin{equation}
    \textit{Lesion load} = \frac{\textit{Number of lesioned voxels that intersect with the tract}}{\textit{Sum of voxels in tract}}
\end{equation}

Motor scores were predicted using ridge regression models with these 6 features as inputs. CST from all bundles were positively correlated with one another and negatively correlated with motor scores (Figure \ref{smatt_pairwise_correlations})
...
\begin{figure}[htp]
\centering
\includegraphics[width=0.8\linewidth]{figures/train_acute_vs_chronic.png}
\caption{Regression coefficients and model performance using Group KFold cross-validation (test/validate on independent imaging site). \textbf{A.} and \textbf{B.} display regression coefficients for regional ChaCo scores (FreeSurfer 86-region atlas, and Shen268-region atlas, respectively). Average feature weights for regions that were selected in at least half of the models (i.e., were included in the model in at least 250/500 outer folds). \textbf{C.} Sensorimotor area tract template feature importance for analysis 2. Includes primary motor cortex (M1), dorsal premotor cortex (PMd), ventral premotor cortex (PMv), supplementary motor area (SMA), pre-supplementary motor area (pre-SMA), primary sensory cortex (S1).}
\label{nemotool}
\end{figure}
